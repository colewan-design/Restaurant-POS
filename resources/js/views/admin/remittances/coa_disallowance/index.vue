<template>
        ...
        <v-dialog v-model="showDeleteDialog">
          ...
          <v-row>
            <v-col cols="12" align="end">
                <router-link :to="{ name: 'remittances.coa_disallowance.create' }" v-if="can('coa-disallowance-create')">
                <v-btn color="#0D47A1">
    ...
    <v-container>
      <v-row>
        <v-col cols="2">
            <v-text-field v-model="searchName" label="Search By Employee Name" @input="filtercoaDisallowances" 
              variant="solo"
          ...
          <v-col cols="2">
            <v-text-field v-model="searchRemarks" label="Search By Remarks" @input="filtercoaDisallowances" 
              variant="solo"
              />
          </v-col>
      ...
      <v-row>
        ...
        :items="coa_disallowances" 
        disable-pagination
        single-line
        :loading="loading">
          <template v-slot:item.actions="{ item }">
            <div class="d-flex align-center">
              <router-link :to="{ name: 'remittances.coa_disallowance.edit', params: { id: item.id} }" v-if="can('coa-disallowance-edit')">
                <v-btn class="mr-3" color="amber" size="small">
                  <v-icon>mdi-pencil</v-icon>Edit
                </v-btn>
              </router-link>
              <v-btn @click="deleteCOADisallowances(item, user.id)" color="red-darken-4" size="small" v-if="can('coa-disallowance-delete')">
  ...
  onMounted(() => {
   ...
   export default {
     data() {
       return {
        ...
        coaDisallowanceHeaders: [
         ...
         { title: 'ND No. 17-002-05-(16)', value: 'nd_no_17_002_05_16', sortable: true },
         { title: 'ND No. 17-003-05-(16)', value: 'nd_no_17_003_05_16', sortable: true },
         { title: 'ND No. 16-004-05-(16)', value: 'nd_no_16_004_05_16', sortable: true },
         { title: 'ND No. 21-018-05-(13)', value: 'nd_no_21_018_05_13', sortable: true },
     ...
     methods: {
      ...
      filtercoaDisallowances() {
        this.loading = true;
  
        // Prepare the API request URL with base parameters
        let apiUrl = `/api/coa_disallowances?page=${this.currentPage}`;
        ...
        axios
          ...
          .then((response) => {
            if (response.data) {
              this.coa_disallowances = response.data.data;
          ...
          .catch((error) => {
            console.error('Error fetching coa disallowances:', error);
          })
          .finally(() => {
            this.loading = false;
          });
      },
      deleteCOADisallowances(landBank_billing, userId) {
      ...
      confirmDelete() {
        if (this.coaDisallowanceToDelete && this.admin_id) {  // Corrected from admin_id to this.admin_id
            ...
            axios.delete(`/api/coa_disallowances/destroy/${coaDisallowanceId}`, {
                params: { admin_id: adminId }
            })
            .then(() => {
                this.coa_disallowances = this.coa_disallowances.filter(comp => comp.id !== coaDisallowanceId);
      ...
      closeDialog() {
          ...
          this.$router.push('/admin/coa_disallowance');
      ...
      getCOADisallowances() {
        ...
        axios.get(`/api/coa_disallowances?page=${this.currentPage}`)
          .then((response) => {
            if (response.data) {
              this.coa_disallowances = response.data.data;
              this.totalPages = response.data.last_page;
