      ...
      <!-- add -->
        <v-dialog v-model="showSuccessDialog">
            <v-card width="600" class="mx-auto" color="teal">
                ...
                <v-card-text>New GSIS Premium has been inserted successfully.</v-card-text>
                <v-card-actions>
      ...
      <!-- edit -->
        <v-dialog v-model="showSuccessEditDialog">
            <v-card width="600" class="mx-auto" color="indigo-darken-3">
                <v-card-title>Success!</v-card-title>
                <v-card-text>New GSIS Premium has been updated successfully.</v-card-text>
                <v-card-actions>
        ...
        <v-dialog v-model="showDeleteDialog">
          <v-card width="600" class="mx-auto" color="red-darken-4">
            ...
            <v-card-text>Are you sure you want to delete this GSIS Premium?</v-card-text>
            <v-card-actions>
              ...
              <v-col cols="12">
                  ...
                  >
                      <v-card-item>
                          ...
                          <v-card-subtitle>
                              This page lists down all BSU Employee GSIS Premium
                          </v-card-subtitle>
                      </v-card-item>
          ...
          <v-row>
            <v-col cols="12" align="end">
                <router-link :to="{ name: 'contributions.gsis_premiums.create' }" v-if="can('gsis-premium-create')">
                <v-btn color="#0D47A1">
        ...
        :loading="loading">
          <template v-slot:item.actions="{ item }">
            <div class="d-flex align-center">
              <router-link :to="{ name: 'contributions.gsis_premiums.edit', params: { id: item.id} }" v-if="can('gsis-premium-edit')">
                <v-btn class="mr-3" color="amber" size="small">
                  <v-icon>mdi-pencil</v-icon>Edit
                </v-btn>
              </router-link>
              <v-btn @click="deletePldtBillings(item, user.id)" color="red-darken-4" size="small" v-if="can('gsis-premium-delete')">
   ...
   export default {
     ...
     methods: {
      filterpldtBillings() {
        ...
        let apiUrl = `/api/gsis_premiums?page=${this.currentPage}`;
        ...
        axios
          ...
          .then((response) => {
            if (response.data) {
              this.gsis_premiums = response.data.data;
          ...
          .catch((error) => {
            console.error('Error fetching GSIS Premiums:', error);
          })
          .finally(() => {
            this.loading = false;
          });
      ...
      confirmDelete() {
        if (this.pldtBillingToDelete && this.admin_id) {  // Corrected from admin_id to this.admin_id
            ...
            axios.delete(`/api/gsis_premiums/destroy/${pldtBillingId}`, {
                params: { admin_id: adminId }
            })
            .then(() => {
                this.gsis_premiums = this.gsis_premiums.filter(comp => comp.id !== pldtBillingId);
                this.showDeleteDialog = false;
            })
            .catch((error) => {
                console.error('Error deleting GSIS Premium:', error);
            });
        }
    },
      ...
      closeDialog() {
          ...
          this.$router.push('/admin/gsis_premiums');
      ...
      getPldtBillings() {
        ...
        axios.get(`/api/gsis_premiums?page=${this.currentPage}`)
          .then((response) => {
            if (response.data) {
              this.gsis_premiums = response.data.data;
              this.totalPages = response.data.last_page;
