      ...
      <!-- add -->
        <v-dialog v-model="showSuccessDialog">
            <v-card width="600" class="mx-auto" color="teal">
                ...
                <v-card-text>New PLDT Billing has been inserted successfully.</v-card-text>
                <v-card-actions>
      ...
      <!-- edit -->
        <v-dialog v-model="showSuccessEditDialog">
            <v-card width="600" class="mx-auto" color="indigo-darken-3">
                <v-card-title>Success!</v-card-title>
                <v-card-text>New PLDT Billing has been updated successfully.</v-card-text>
                <v-card-actions>
        ...
        <v-dialog v-model="showDeleteDialog">
          <v-card width="600" class="mx-auto" color="red-darken-4">
            ...
            <v-card-text>Are you sure you want to delete this PLDT Billing?</v-card-text>
            <v-card-actions>
              ...
              <v-col cols="12">
                  ...
                  >
                      <v-card-item>
                          ...
                          <v-card-subtitle>
                              This page lists down all BSU Employee PLDT Billing
                          </v-card-subtitle>
                      </v-card-item>
          ...
          <v-row>
            <v-col cols="12" align="end">
                <router-link :to="{ name: 'billings.pldt_billings.create' }" v-if="can('pldt-billing-create')">
                <v-btn color="#0D47A1">
        ...
        :loading="loading">
          <template v-slot:item.actions="{ item }">
            <div class="d-flex align-center">
              <router-link :to="{ name: 'billings.pldt_billings.edit', params: { id: item.id} }" v-if="can('pldt-billing-edit')">
                <v-btn class="mr-3" color="amber" size="small">
                  <v-icon>mdi-pencil</v-icon>Edit
                </v-btn>
              </router-link>
              <v-btn @click="deletePldtBillings(item, user.id)" color="red-darken-4" size="small" v-if="can('pldt-billing-delete')">
   ...
   export default {
     data() {
       return {
        ...
        pldtBillingHeaders: [
         ...
         { title: 'SOA Number', value: 'soa_number', sortable: true },
         { title: 'Bill Date', value: 'bill_date', sortable: true },
         { title: 'Payment Due Date', value: 'payment_due_date', sortable: true },
         { title: 'Total Amount Due', value: 'total_amount_due', sortable: true },

         { title: 'Previous Balance', value: 'previous_balance', sortable: true },
         { title: 'Payments', value: 'payments', sortable: true },
         { title: 'Adjustments', value: 'adjustments', sortable: true },
         { title: 'Past Due Amount', value: 'past_due_amount', sortable: true },
         { title: 'Current Amount', value: 'current_amount', sortable: true },
     ...
     methods: {
      filterpldtBillings() {
        ...
        let apiUrl = `/api/pldt_billings?page=${this.currentPage}`;
        ...
        axios
          ...
          .then((response) => {
            if (response.data) {
              this.pldt_billings = response.data.data;
      ...
      confirmDelete() {
        if (this.pldtBillingToDelete && this.admin_id) {  // Corrected from admin_id to this.admin_id
            ...
            axios.delete(`/api/pldt_billings/destroy/${pldtBillingId}`, {
                params: { admin_id: adminId }
            })
            .then(() => {
                this.pldt_billings = this.pldt_billings.filter(comp => comp.id !== pldtBillingId);
      ...
      closeDialog() {
          ...
          this.$router.push('/admin/pldt_billings');
      ...
      getPldtBillings() {
        ...
        axios.get(`/api/pldt_billings?page=${this.currentPage}`)
          .then((response) => {
            if (response.data) {
              this.pldt_billings = response.data.data;
              this.totalPages = response.data.last_page;
