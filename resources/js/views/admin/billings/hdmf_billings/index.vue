<template>
    ...
    <!-- add -->
      <v-dialog v-model="showSuccessDialog">
          <v-card width="600" class="mx-auto" color="teal">
              ...
              <v-card-text>New HDMF Billing has been inserted successfully.</v-card-text>
              <v-card-actions>
    ...
    <!-- edit -->
      <v-dialog v-model="showSuccessEditDialog">
          <v-card width="600" class="mx-auto" color="indigo-darken-3">
              <v-card-title>Success!</v-card-title>
              <v-card-text>New HDMF Billing has been updated successfully.</v-card-text>
              <v-card-actions>
      ...
      <v-dialog v-model="showDeleteDialog">
        <v-card width="600" class="mx-auto" color="red-darken-4">
          ...
          <v-card-text>Are you sure you want to delete this HDMF Billing?</v-card-text>
          <v-card-actions>
            ...
            <v-col cols="12">
                ...
                >
                    <v-card-item>
                        ...
                        <v-card-subtitle>
                            This page lists down all BSU Employee HDMF Billing
                        </v-card-subtitle>
                    </v-card-item>
        ...
        <v-row>
          <v-col cols="12" align="end">
              <router-link :to="{ name: 'billings.hdmf_billings.create' }" v-if="can('hdmf-billing-create')">
              <v-btn color="#0D47A1">
  ...
  <v-container>
    <v-row>
      <v-col cols="2">
          <v-text-field v-model="searchName" label="Search By Employee Name" @input="filterhdmfBillings" 
            variant="solo"
            prepend-icon="mdi-clipboard-text-search-outline"
            />
        </v-col>
        <v-col cols="2">
          <v-text-field v-model="searchPresentMeterReading" label="Search By Branch" @input="filterhdmfBillings" 
            variant="solo"
            />
        </v-col>
        <v-col cols="2">
          <v-text-field v-model="searchEnergyUsed" label="Search By Account Number" @input="filterhdmfBillings" 
            variant="solo"
        ...
        <v-col cols="2">
          <v-text-field v-model="searchAmountDue" label="Search By Date Granted" @input="filterhdmfBillings" 
            variant="solo"
            />
        </v-col>

        <v-col cols="2">
          <v-text-field v-model="searchRemarks" label="Search By Remarks" @input="filterhdmfBillings" 
            variant="solo"
            />
        </v-col>
      </v-row>
      ...
      :loading="loading">
        <template v-slot:item.actions="{ item }">
          <div class="d-flex align-center">
            <router-link :to="{ name: 'billings.hdmf_billings.edit', params: { id: item.id} }" v-if="can('hdmf-billing-edit')">
              <v-btn class="mr-3" color="amber" size="small">
                <v-icon>mdi-pencil</v-icon>Edit
              </v-btn>
            </router-link>
            <v-btn @click="deleteHdmfBillings(item, user.id)" color="red-darken-4" size="small" v-if="can('hdmf-billing-delete')">
 ...
 export default {
   data() {
     return {
      ...
      hdmfBillingHeaders: [
       ...
       { title: 'Pag-IBIG ID/RTN', value: 'rtn', sortable: true },
       { title: 'Employee', value: 'employee_name', sortable: true },
       { title: 'Application Number', value: 'application_number', sortable: true },
   ...
   methods: {
    filterhdmfBillings() {
      ...
      let apiUrl = `/api/hdmf_billings?page=${this.currentPage}`;
      ...
      axios
        ...
        .then((response) => {
          if (response.data) {
            this.hdmf_billings = response.data.data;
        ...
        .catch((error) => {
          console.error('Error fetching hdmf billings:', error);
        })
        .finally(() => {
          this.loading = false;
        });
    },
    deleteHdmfBillings(hdmf_billing, userId) {
    ...
    confirmDelete() {
      if (this.hdmfBillingToDelete && this.admin_id) {  // Corrected from admin_id to this.admin_id
    ...
    getHdmfBillings() {
      ...
      axios.get(`/api/hdmf_billings?page=${this.currentPage}`)
        .then((response) => {
          if (response.data) {
            this.hdmf_billings = response.data.data;
            this.totalPages = response.data.last_page;
