        ...
        <v-dialog v-model="showDeleteDialog">
          ...
          <v-row>
            <v-col cols="12" align="end">
                <router-link :to="{ name: 'customers.create' }" v-if="can('customer-create')">
                <v-btn color="#0D47A1">
  ...
  onMounted(() => {
   ...
   export default {
     ...
     methods: {
      ...
      deleteCustomers(customer, userId) {
      ...
      confirmDelete() {
        if (this.customerToDelete && this.admin_id) {  // Corrected from admin_id to this.admin_id
            const customerId = this.customerToDelete.id;
            const adminId = this.admin_id;
  
            axios.delete(`/api/customers/destroy/${customerId}`, {
