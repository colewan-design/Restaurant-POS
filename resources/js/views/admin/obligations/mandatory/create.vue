<template>
    <v-divider></v-divider>
    <v-divider></v-divider>
    <v-divider></v-divider>
    <v-container>
        <v-row>
            <v-sheet width="1200" class="mt-2 mx-auto p-2">
                <v-form @submit.prevent>
                    <v-text-field
                        variant="solo"
                        v-model="mandatory_deduction_name"
                        label="Mandatory Deduction Name"
                        :rules="[
                            v => !!v || 'Mandatory Deduction Name is required'
                        ]"
                    ></v-text-field>
                    <v-text-field
                        variant="solo"
                        v-model="mandatory_value"
                        label="Mandatory Value"
                        :rules="[
                            v => !!v || 'Mandatory Value is required',
                            v => /^\d+(\.\d+)?$/.test(v) || 'Mandatory Value must be a valid number'
                        ]"
                    ></v-text-field>
                    <v-autocomplete
                        v-model="mandatory_type"
                        :items="['percentage', 'flat amount']"
                        label="Mandatory Type"
                        item-text="text"
                        item-value="value"
                        :rules="[
                            v => !!v || 'Mandatory Type is required',
                            v => this.isMandatoryTypeValid(v) || 'Mandatory Type must be either percentage or flat amount'
                        ]"
                    ></v-autocomplete>

                    <v-text-field
                        variant="solo"
                        v-model="description"
                        label="Description"
                    ></v-text-field>
                    <v-btn
                        type="submit"
                        class="mt-2 mr-2"
                        @click="handleSubmit"
                        :disabled="isDuplicate || !mandatory_deduction_name.trim()"
                        color="teal"
                    >
                        Submit
                    </v-btn>
                    <v-btn class="mt-2" color="gray" @click="clearFields">Clear</v-btn>
                </v-form>


            </v-sheet>
        </v-row>
    </v-container>
</template>

<script>
export default {
    data() {
        return {
            mandatory_deduction_name: '',
            mandatory_value: '',
            mandatory_type: '',
            description: '',
            active: '',
            existingMandatoryDeductions: [],
        };
    },

    computed: {
        isDuplicate() {
            return this.existingMandatoryDeductions.includes(this.mandatory_deduction_name.trim());
        },
    },

    methods: {
        isMandatoryTypeValid(type) {
            const lowerCaseType = type.toLowerCase();
            return ['percentage', 'flat amount'].includes(lowerCaseType);
        },
        async fetchExistingMandatoryDeductions() {
            try {
                const response = await fetch('/api/mandatory_deductions');

                if (response.ok) {
                    const projects = await response.json();
                    this.existingMandatoryDeductions = projects.map(mandatory => mandatory.mandatory_deduction_name);
                } else {
                    console.error('Failed to fetch existing projects.');
                }
            } catch (error) {
                console.error('Error:', error);
            }
        },

        async handleSubmit() {
            await this.fetchExistingMandatoryDeductions();

            if (!this.isDuplicate) {
                try {
                    const formData = {
                        mandatory_deduction_name: this.mandatory_deduction_name,
                        mandatory_value: this.mandatory_value,
                        mandatory_type: this.mandatory_type,
                        description: this.description,
                    };

                    const response = await fetch('/api/mandatory_deductions/create', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify(formData),
                    });

                    if (response.ok) {
                        console.log('Data inserted successfully!');
                        this.$router.push({ name: 'admin.obligations.mandatory.index', query: { showSuccessDialog: 'true' } });
                    } else {
                        console.error('Failed to insert data.');
                    }
                } catch (error) {
                    console.error('Error:', error);
                }
            } else {
                // Show a message that the project name is a duplicate
                console.log('Mandatory Deduction name is a duplicate.');
            }
        },

        clearFields() {
            this.mandatory_deduction_name = '';
        },
    },

    created() {
        this.fetchExistingMandatoryDeductions();
    },
};
</script>
